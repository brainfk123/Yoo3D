...
void UNeuronLiveLinkRemapAsset::SetLiveLinkSubjectName(FLiveLinkSubjectName LiveLinkSubjectName)
{
...
float UNeuronLiveLinkRemapAsset::GetSkeletonHeightScale(FName RootBoneName) const
{
	float SkeletonPivotHeight = 1.0f;
	float LiveLinkSkeletonHeight = 1.0f;
	
	if (GetWorld())
	{
		if (FNeuronLiveLinkModule::Get().GetLiveLinkSkeletonHeight.IsBound())
		{
			LiveLinkSkeletonHeight = FNeuronLiveLinkModule::Get().GetLiveLinkSkeletonHeight.Execute(SubjectName);
		}
	}
	
	if (auto AnimInstance = Cast<UAnimInstance>(GetOuter()))
	{
		USkeleton* Skeleton = AnimInstance->CurrentSkeleton;
		const FReferenceSkeleton& SkeletonRef = Skeleton->GetReferenceSkeleton();
		int32 Index = SkeletonRef.FindRawBoneIndex(RootBoneName);
		if (SkeletonRef.GetRawRefBonePose().IsValidIndex(Index))
		{
			FTransform RootTransform = SkeletonRef.GetRawRefBonePose()[Index];
			SkeletonPivotHeight = RootTransform.GetLocation().Z;
		}
	}

	// Average body size
	return SkeletonPivotHeight / (0.53f * LiveLinkSkeletonHeight);
}
	...
	if (bWithDisplacement)
	{
		for (int32 i = 0; i < TransformedBoneNames.Num( ); ++i)
		{
			...
			if (InFrameData->Transforms.IsValidIndex( i ))
			{
				...
				BoneTransform.SetLocation(BoneTransform.GetLocation() * GetSkeletonHeightScale(HipsNameInTargetSkeleton));
